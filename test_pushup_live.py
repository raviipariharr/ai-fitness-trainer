...
Purpose:
    Tests exercises/pushup.py against a real, live camera feed. Do a
    few real push-ups (camera roughly side-on works best, so both the
    arm and the hip-to-ankle line are visible) and confirm the count
    only increases once per push-up, and that the form feedback reacts
    to a deliberately bad rep (e.g. sagging your hips).

Usage:
    python test_pushup_live.py
...
WINDOW_NAME = "Push-up Counter Test"


def draw_status(frame, status: dict) -> None:
    cv2.putText(frame, f"Push-ups: {status['rep_count']}", (10, 40),
    ...
    if status["body_feedback"]:
        cv2.putText(frame, status["body_feedback"], (10, 140),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
    elif status["last_rep_feedback"]:
...
def main() -> None:
    ...
    print("Push-up test started. Camera side-on works best. Press 'q' to quit, 'r' to reset.")
    ...
    try:
        while True:
            ...
            status = pushup_counter.update(coordinates) if coordinates else {
                "rep_count": pushup_counter.rep_count, "state": pushup_counter.state.value,
                "elbow_angle": None, "side": None,
                "body_feedback": None, "last_rep_feedback": pushup_counter.last_rep_feedback,
            }
        ...
        print(f"Test finished. Final push-up count: {pushup_counter.rep_count}")
